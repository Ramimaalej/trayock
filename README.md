...
- **Blocking**: `EvdevDevice.grab()` on every detected keyboard and pointer
  device — all input events are consumed by inputlock and never reach the
  desktop.
...
Device classification (`inputlock/classify.py`):
...
inputlock for your user, adds you to the `input` group, installs the launcher
...
cd trayock
python3 -m venv --system-site-packages ~/.local/share/inputlock
~/.local/share/inputlock/bin/pip install .
```
...
enough** — do *not* run inputlock as root.
...
inputlock --list-devices     # every row should say "ok"
```
...
```bash
sudo tee /etc/udev/rules.d/99-inputlock.rules <<'EOF'
...
When permissions are missing, inputlock **refuses to fake a lock**: it shows
an error dialog with the exact devices it could not open, and stays
unlocked. `inputlock --list-devices` prints the same diagnosis.

## Usage

```bash
inputlock                     # start the tray app
inputlock --mode keyboard     # default mode: grab keyboards only
inputlock --mode mouse        # default mode: grab pointers only
inputlock --list-devices      # diagnose devices/permissions, then exit
inputlock --keyboard-only     # alias for --mode keyboard
inputlock --escape-seconds 3  # change the escape hold time
inputlock -v                  # debug logging
```
...
1. **Escape combo (always available):** hold **`Ctrl+Alt+Shift+L` for
   2 seconds**. inputlock reads it itself from the evdev devices — no
...
3. **Last resort:** `pkill -f inputlock` from a TTY, Ctrl+Alt+F3, or another
   machine — a released grab always returns input, even if inputlock crashes.
...
```bash
inputlock --install-desktop   # app-grid / menu launcher + padlock icon
inputlock --remove-desktop    # ... and take it away again
inputlock --install-autostart # start inputlock automatically at login
inputlock --remove-autostart  # ... and stop
```
...
- `packaging/inputlock.desktop` — launcher with a plain `Exec=inputlock`
- `packaging/99-inputlock.rules` — udev rule granting the `input` group access
...
- The lock itself is **kernel-level** (`EVIOCGRAB` on the evdev nodes), so it
  ...
  compositor too.
- The **escape combo is processed by inputlock itself** (it reads the grabbed
...
- Caveats under GNOME/Wayland:
  ...
  - uinput-based virtual keyboards (e.g. `ydotool`, on-screen keyboards) are
    separate devices and are only blocked if inputlock grabs them too
  ...
  - If inputlock **crashes or is killed**, the kernel closes the file
    ...
    (all keyboards unplugged): inputlock unlocks itself instead of claiming
...
If `AyatanaAppIndicator3` can't be imported, inputlock tries `pystray`
(install with `pip install 'inputlock[pystray]'`), which supports
...
packaging/
    inputlock.desktop    ready-made launcher for system-wide installs
    99-inputlock.rules   udev rule for the 'input' group
src/inputlock/
    __init__.py          version
    __main__.py          python -m inputlock
    ...
    data/inputlock.desktop   template the installer fills in
...
| pystray fallback not visible | GNOME needs the same AppIndicator extension; install `inputlock[pystray]` only provides the library, not the shell support. |
